...
class AddExpense extends Component {
  ...
  async handleSave(event) {
    ...
    if (this.validateForm()) {
      ...
      if (goal) {
        goal.spentAmount =
                    parseFloat(goal.spentAmount) +
                    parseFloat(selectedExpensePrice);
        goalAPI.update(goal).catch((err) =>
          console.log(err));
      }
      transactionAPI
  ...
  validateForm() {
    const v_expense = this.state.selectedExpense;
    const errors = {};
