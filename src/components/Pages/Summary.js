...
class Summary extends Component {
    ...
    async componentDidMount() {
        ...
        await goalAPI.get({ userId: obj.token, mmyyID: 'all' }).then((json) => {
        ...
        transactionAPI.getSpendingTotal(obj.token, 'all').then((json) => {
            if (json[0] === undefined) {
                ...
                this.setState({
                    totalSpent: (json[0].spendingTotal / 100).toFixed(2),
                });
            }
        });
        transactionAPI.getIncomeTotal(obj.token, 'all').then((json) => {
            if (json[0] === undefined) {
                ...
                this.setState({
                    totalEarned: (json[0].incomeTotal / 100).toFixed(2),
                });
            }
        });
    }
        ...
        for (var catIdx in this.state.disCats) {
            ...
            for (var idx in this.state.allCatTotals) {
                if (card.category == this.state.allCatTotals[idx]._id) {
                    card.totalSpent = (
                        this.state.allCatTotals[idx].totals / 100
                    ).toFixed(2);
                }
            }
            ...
            for (var allIdx in this.state.allGoalsEver) {
                if (
                    card.category === this.state.allGoalsEver[allIdx].category
                ) {
    ...
    cardLogic(cards) {
        for (var i in cards) {
            ...
            card.percent_metGoal =
                (card.timesmetGoal / card.timeswithGoal) * 100;
            ...
            <Card key={key}>
                <Accordion.Toggle as={Card.Header} eventKey={key}>
                    <Row>
                        ...
                        <Col>
                            <h6>You&apos;ve Spent ${card.totalSpent}</h6>
                        </Col>
                    </Row>
                </Accordion.Toggle>
                <Accordion.Collapse eventKey={key}>
                    <Card.Body>
                        <p>
                            You&apos;ve had this goal for {card.timeswithGoal}{' '}
                            months
                        </p>
                        <p>
                            {card.percent_metGoal}% of the time you meet your
                            goal
                        </p>
            ...
            <div>
                ...
                <Container>
                    ...
                    <Row>
                        ...
                        <Col>
                            <Row>
                                How are the goals that you&apos;ve set working
                                out?
                            </Row>
                            <div>
                                <Accordion>
