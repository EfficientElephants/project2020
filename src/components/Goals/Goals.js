...
class Goals extends Component {
    ...
    if (obj && obj.token) {
      ...
      if (
        currentGoals.length === 0 &&
                oldGoals.length !== 0 &&
                this.state.date.getDate() === 2
      ) {
        const updatedGoals = await this.createNewDatabases(oldGoals).then(
          () => {
            const updatedGoals = this.getGoals(this.state.mmyyID);
            return updatedGoals;
          }
        );
  ...
  async handleSave(event) {
    ...
    if (this.validateForm()) {
      ...
      await goalAPI.update(this.state.selectedGoal).then(() => {
        this.setState({
          selectedGoal: null,
          render: true,
        });
  ...
  validateForm() {
    const v_goal = this.state.selectedGoal;
    const errors = {};
