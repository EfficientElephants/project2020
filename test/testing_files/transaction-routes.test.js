...
describe('getSpendingTotal', () => {
  it('should return spending total of all transactions', (done) => {
    ...
    const totalsAll = (
      parseFloat(transactions[0].price) + parseFloat(transactions[1].price)
    ).toFixed(2);
    Transaction.insertMany(transactions).then(() => {
      chai
        .request(app)
        ...
        .end((err, res) => {
          res.body[0].spendingTotal = (res.body[0].spendingTotal / 100).toFixed(
            2
          );
          expect(res.statusCode).to.equal(200);
          expect(res.body).to.be.an('array');
    ...
    Transaction.insertMany(transactions).then(() => {
      chai
        ...
        .end((err, res) => {
          ...
          expect(res.body[0])
            .excluding(['date', 'id', '_id', '__v', 'createdAt', 'updatedAt'])
